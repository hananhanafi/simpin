            ...
            <table class="table table-bordered table-striped simulasi">
                ...
                <tbody>
                    ...
                    @for ($i = 1; $i <= $bulan; $i++)
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="hidden" name="simulasi[ssb][rollover][blnThn][{{ $i }}]"
                                    value="{{ $rangeBulan[0][$i]['bulan'] }}">
                                {{ isset($rangeBulan[0][$i]['bulan']) ? $rangeBulan[0][$i]['bulan'] : '-' }}
                            </td>
                            <td class="text-center">
                                <input type="hidden" name="simulasi[ssb][rollover][jlhHari][{{ $i }}]"
                                    value="{{ $rangeBulan[0][$i]['jlh_hari'] }}">
                                {{ isset($rangeBulan[0][$i]['jlh_hari']) ? $rangeBulan[0][$i]['jlh_hari'] : '-' }}
                            </td>
                            <td class="text-right">
                                <input type="hidden" name="simulasi[ssb][rollover][saldo][{{ $i }}]"
                                    value="{{ $saldoBunga }}">
                                {{ number_format($saldoBunga, 0, ',', '.') }}
                            </td>
                            <td class="text-center">
                                <input type="hidden" name="simulasi[ssb][rollover][bungaHarian][{{ $i }}]"
                                    value="{{ $bunga }}">
                                {{ $bunga }}%
                            </td>
                            @php
                            ...
                            @endphp
                            <td class="text-right">
                                <input type="hidden" name="simulasi[ssb][rollover][jlhBunga][{{ $i }}]"
                                    value="{{ $jlhBunga }}">
                                {{ number_format($jlhBunga, 0, ',', '.') }}
                            </td>
                            <td class="text-right">
                                <input type="hidden" name="simulasi[ssb][rollover][bungapph][{{ $i }}]"
                                    value="{{ $jlhBungaPph }}">
                                {{ number_format($jlhBungaPph, 0, ',', '.') }}
                            </td>
                            <td class="text-right">
                                <input type="hidden"
                                    name="simulasi[ssb][rollover][jumlahditerima][{{ $i }}]"
                                    value="{{ $jlhDiterima }}">
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="simulasi[ssb][rollover][totalJumlahBunga]"
                            value="{{ $totalJumlahBunga }}">
                    </tr>
                </tbody>
            </table>
                ...
                <tbody>
                    ...
                    @for ($i = 1; $i <= $bulan; $i++)
                        <tr>
                            ...
                            <td class="text-center">
                                {{ isset($rangeBulan[0][$i]['bulan']) ? $rangeBulan[0][$i]['bulan'] : '-' }}
                                <input type="hidden" name="simulasi[ssb][dibayar][blnThn][{{ $i }}]"
                                    value="{{ $rangeBulan[0][$i]['bulan'] }}">
                            </td>
                            <td class="text-center">
                                {{ isset($rangeBulan[0][$i]['jlh_hari']) ? $rangeBulan[0][$i]['jlh_hari'] : '-' }}
                                <input type="hidden" name="simulasi[ssb][dibayar][jlhHari][{{ $i }}]"
                                    value="{{ $rangeBulan[0][$i]['jlh_hari'] }}">
                            </td>
                            <td class="text-right">
                                {{ number_format($saldo, 0, ',', '.') }}
                                <input type="hidden" name="simulasi[ssb][dibayar][saldo][{{ $i }}]"
                                    value="{{ $saldo }}">
                            </td>
                            <td class="text-center">
                                {{ $bunga }}%
                                <input type="hidden" name="simulasi[ssb][dibayar][bungaHarian][{{ $i }}]"
                                    value="{{ $bunga }}">
                            </td>
                            @php
                                $totalHari = $rangeBulan['totalHari'];
                            ...
                            <td class="text-right">
                                {{ number_format($jlhBunga, 0, ',', '.') }}
                                <input type="hidden" name="simulasi[ssb][dibayar][jlhBunga][{{ $i }}]"
                                    value="{{ $jlhBunga }}">
                            </td>
                            <td class="text-right">
                                <input type="hidden" name="simulasi[ssb][dibayar][bungapph][{{ $i }}]"
                                    value="{{ $jlhBungaPph }}">
                                {{ number_format($jlhBungaPph, 0, ',', '.') }}
                            </td>
                            <td class="text-right">
                                <input type="hidden"
                                    name="simulasi[ssb][dibayar][jumlahditerima][{{ $i }}]"
                                    value="{{ $jlhDiterima }}">
                                {{ number_format($jlhDiterima, 0, ',', '.') }}
                            </td>
                        </tr>
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="simulasi[ssb][dibayar][totalJumlahBunga]"
                            value="{{ $totalJumlahBunga }}">
